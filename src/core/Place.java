...
public class Place {
	...
	public void addInsect (Ant ant) {
		if (this.ant == null) {
		...
		else if (ant instanceof Containing && !(this.ant instanceof Containing)){
			ant.setPlace(this);
			((Containing)ant).addAnt(this.ant);
			this.ant=ant;
		}
			...
			if (ant instanceof QueenAnt)
				QueenAnt.nbQueenAnt-=1;
		}
	}
	
	public void setNextQueen(){
		this.isNextQueen=true;
	}
	
	public boolean isNextQueen(){
		return isNextQueen;
	}
	/**
	...
	public void removeInsect (Ant ant) {
		if (this.ant instanceof Containing){
			if (this.ant ==ant){
				Ant temp =((Containing) this.ant).getAnt();
				this.ant=temp;
				this.ant.setPlace(this);
			}
			else if (((Containing) this.ant).getAnt()==ant)
				((Containing) this.ant).removeAnt(ant);
		}
		else if (this.ant == ant && !(this.ant instanceof QueenAnt)) {
			this.ant = null;
			ant.setPlace(null);
		}
		else if (this.ant instanceof QueenAnt)
			System.out.println("QueenAnt can't be deleted");
		else {
