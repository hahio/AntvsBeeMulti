...
public class AntColony {
	...
	public void reduceFood(int price){
		food-=price;
	}
	/**
	...
	public void deployAnt (Place place, Ant ant) {
		if (place instanceof Water && !ant.getWatersafe())
			System.out.println(ant + " is gonna drown " );
		else if (ant instanceof QueenAnt && QueenAnt.nbQueenAnt>2)
			System.out.println(ant+ " 2 Queen is 2 much");
		else if (food >= ant.getFoodCost()) {
			place.addInsect(ant);
	...
	public ArrayList<Ant> getAllAnts () {
		...
		for (Place p : places) {
			if (p.getAnt() != null) {
				if (p.getAnt() instanceof Containing && ((Containing) (p.getAnt())).getAnt()!=null)
